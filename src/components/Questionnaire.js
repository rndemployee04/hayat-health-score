    ...
    if (!isStarted) {
        return (
            <div style={{ textAlign: 'center', padding: '3rem 1rem', animation: 'fadeIn 0.5s' }}>
                <h2 style={{ color: primaryColor, fontFamily: 'Outfit, sans-serif', fontSize: '2rem', marginBottom: '1rem' }}>
                ...
                >
                    Start My 60-Second Health Score
                </button>
            </div>
        );
    }
        ...
        <div ref={containerRef} style={{ position: 'relative', padding: '2rem', textAlign: 'left', animation: 'fadeIn 0.4s ease-in-out', minHeight: '550px', display: 'flex', flexDirection: 'column' }}>
            ...
            {showExitIntent && (
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(251, 245, 232, 0.95)', zIndex: 10, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '2rem', textAlign: 'center', borderRadius: '12px' }}>
                    <h3 style={{ color: primaryColor, marginBottom: '1rem', fontFamily: 'Outfit, sans-serif' }}>Wait! Don't leave just yet.</h3>
                    ...
                    <button onClick={() => setShowExitIntent(false)} style={{ backgroundColor: primaryColor, color: '#FFF', padding: '0.75rem 2rem', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '1.1rem', fontFamily: 'Outfit, sans-serif' }}>Continue Assessment</button>
                </div>
            )}

            <ProgressBar currentStep={currentStepIndex + 1} totalSteps={questions.length} />

            <h3 style={{ color: primaryColor, marginBottom: '0.5rem', fontFamily: 'Outfit, sans-serif', fontSize: '1.4rem' }}>{currentQuestion.title}</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '2rem', flex: 1 }}>
                {currentQuestion.type === 'checkbox' && currentQuestion.options.map((option) => (
                    <label key={option} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', padding: '0.75rem', border: '1px solid', borderColor: currentAnswer.includes(option) ? primaryColor : '#DCD7C9', borderRadius: '6px', backgroundColor: currentAnswer.includes(option) ? '#f0f9f4' : '#fff', outline: 'none', userSelect: 'none' }}>
                ...
                {currentQuestion.type === 'radio' && currentQuestion.options.map((option) => (
                    <label key={option} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', padding: '0.75rem', border: '1px solid', borderColor: currentAnswer === option ? primaryColor : '#DCD7C9', borderRadius: '6px', backgroundColor: currentAnswer === option ? '#f0f9f4' : '#fff', outline: 'none', userSelect: 'none' }}>
                ...
                {currentQuestion.type === 'slider' && (
                    <div style={{ padding: '1rem 0' }}>
                        ...
                        <div style={{ textAlign: 'center', marginTop: '1rem', fontSize: '1.2rem', fontWeight: 'bold', color: primaryColor }}>{currentAnswer}</div>
                    </div>
                )}
            </div>

            {currentQuestion.insight && (
                <div style={{ backgroundColor: '#FBF5E8', padding: '1rem', borderRadius: '6px', borderLeft: '4px solid #f0ad4e', marginBottom: '2rem' }}>
                    <p style={{ margin: 0, fontStyle: 'italic', color: '#666', fontFamily: 'Lexend, sans-serif', fontSize: '0.95rem' }}>
                        <strong>Insight:</strong> {currentQuestion.insight}
                    </p>
                </div>
            )}
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
                ...
                <button onClick={handleNext} disabled={isNextDisabled()} style={{ backgroundColor: primaryColor, color: '#FFF', padding: '0.75rem 1.5rem', border: 'none', borderRadius: '4px', cursor: isNextDisabled() ? 'not-allowed' : 'pointer', opacity: isNextDisabled() ? 0.6 : 1, fontSize: '1.1rem', fontFamily: 'Outfit, sans-serif', flex: 2 }}>
